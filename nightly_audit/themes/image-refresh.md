...
- `Permission denied` (rc=126) は許可 deny ではなく実行ビット欠け。ハーネス側の欠陥として
  要相談に回す (自分で chmod しない)。
- rc=137 (SIGKILL) は OOM ではなく**並行作業の一括コンテナ操作による巻き添え kill** の
  可能性が高い (2026-07-17 に実例)。`docker events --since 30m --until 1s` で
  kill イベントを確認し、事実をレポートへ。1回だけ再試行してよい。
- 「権限 deny」と結論する場合は、ツールが返した**拒否メッセージ原文をレポートに引用**する
  こと (引用できないなら deny と断定しない)。手順のコマンドは**一字一句そのまま**打つ。
